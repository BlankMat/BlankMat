...
private:
	...
	static std::vector<Texture*> LoadMaterialTextures(Scene* scene, const aiMaterial* mat, aiTextureType exportType, TextureType type)
	{
		std::vector<Texture*> textures;
		for (unsigned int i = 0; i < mat->GetTextureCount(exportType); i++)
		{
			aiString str;
			mat->GetTexture(exportType, i, &str);
...
public:
	...
	static void SaveModel(Scene* scene, const std::string& path, const glm::vec3& startPos = glm::vec3(0.0f), const glm::vec3& startRot = glm::vec3(0.0f), const glm::vec3& startScale = glm::vec3(1.0f))
	{
		return;
