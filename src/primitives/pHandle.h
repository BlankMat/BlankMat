...
public:
	...
	PHandle(float len, Shader* shader, Material* defaultMat, State* state, float lineWidth, bool drawOver,
		...
		: IEntity(shader, defaultMat, defaultMat, state, drawOver, pos, rot, scale)
	{
		mXHandle = new PLine(glm::vec3(len*0.05f,0,0), glm::vec3(len,0,0), shader, new Material(xColor), defaultMat, state, lineWidth, drawOver);
		mYHandle = new PLine(glm::vec3(0,len*0.05f,0), glm::vec3(0,len,0), shader, new Material(yColor), defaultMat, state, lineWidth, drawOver);
		mZHandle = new PLine(glm::vec3(0,0,len*0.05f), glm::vec3(0,0,len), shader, new Material(zColor), defaultMat, state, lineWidth, drawOver);
		mAllHandle = new PWireCube(len*0.1f, shader, new Material(allColor), defaultMat, state, lineWidth*0.5f, drawOver);
