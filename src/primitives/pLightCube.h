...
public:
	...
	PLightCube(float size, Shader* shader, Material* defaultMat, State* state, glm::vec3 pos = glm::vec3(1.0f), glm::vec3 dir = glm::vec3(-1.0f), glm::vec3 color = glm::vec3(1.0f),
		float kd = 1.0f, float ka = 0.1f, float ks = 0.5f, bool gamma = false)
        : PCube(size, shader, new Material(color), defaultMat, state, 0.0f, false, pos), Light(pos, dir, color, kd, ka, ks, gamma) {}

    PLightCube(float size, Shader* shader, Material* defaultMat, State* state, Config* config)
        : PLightCube(size, shader, defaultMat, state, config->GetVec("pos"), config->GetVec("dir"), config->GetVec("color"), 
