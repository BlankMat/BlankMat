...
glm::vec3 OldMesh::GetUp() { return up; }
glm::vec3 OldMesh::GetRight() { return right; }
glm::vec3 OldMesh::GetForward() { return forward; }

void OldMesh::Translate(glm::vec3 _deltaPos) { pos += _deltaPos; }
void OldMesh::Rotate(glm::vec3 _deltaRot) { rotation += _deltaRot; }
void OldMesh::Scale(glm::vec3 _deltaScale) { scale = glm::max(glm::vec3(scale.x * _deltaScale.x, scale.y * _deltaScale.y, scale.z * _deltaScale.z), MIN_SCALE); }

glm::vec3 OldMesh::GetPos() { return pos; }
glm::vec3 OldMesh::GetRotation() { return rotation; }
glm::vec3 OldMesh::GetScale() { return scale; }
glm::vec3 OldMesh::GetPivot() { return pivot; }

void OldMesh::SetPos(glm::vec3 _pos) { pos = _pos; }
void OldMesh::SetRotation(glm::vec3 _rot) { rotation = _rot; }
void OldMesh::SetScale(glm::vec3 _scale) { scale = glm::max(_scale, MIN_SCALE); }

// Translate the given verts by the given vector
void OldMesh::Translate(std::set<int>& _verts, glm::vec3 _deltaPos)
{
...
void OldMesh::Rotate(std::set<int>& _verts, glm::vec3 _deltaRot, glm::vec3 _pivot)
{
...
void OldMesh::Scale(std::set<int>& _verts, glm::vec3 _deltaScale, glm::vec3 _pivot)
{
	for (auto iter = _verts.begin(); iter != _verts.end(); ++iter) {
...
// ------------------------------
void OldMesh::AddFace(Face _face) { faces.push_back(_face); }
int OldMesh::AddVert(Vertex _vert) { return AddVert(lastVertIndex + 1, _vert); }

int OldMesh::AddVert(int _index, Vertex _vert)
{
...
Face OldMesh::GetFace(size_t index) { return faces[index]; }
Vertex OldMesh::GetVert(size_t index) { return verts[(int)index]; }
...
int OldMesh::GetVertCount()
{
	return (int)verts.size();
}

int OldMesh::GetIndexCount()
{
	int count = 0;
...
std::vector<Face>& OldMesh::GetFaces()
{
	return faces;
}

std::vector<Triangle>& OldMesh::GetTris(std::vector<Triangle>& _tris)
{
...
std::vector<std::string>& OldMesh::GetMatsForVert(std::vector<std::string>& _mats, int vertId)
{
	for (int i = 0; i < faces.size(); i++) {
...
std::unordered_map<int, Vertex>& OldMesh::GetVerts()
{
	return verts;
}

std::string OldMesh::GetName() { return name; }
void OldMesh::SetName(std::string _name) { name = _name; }

void OldMesh::RecalculateNormals()
{
...
int OldMesh::GetVertexModel() { return 1; }
